// main.js — сервер «Komfort» без библиотеки cors
...
// ===================== Ручная замена CORS =====================
// Если кросс-запросы не нужны, этот middleware можно удалить полностью.
//
// По умолчанию сервер отражает любой Origin, как было в версии с cors({ origin: true }).
// Если нужен строгий список разрешённых адресов, задайте переменную окружения:
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      status: user.status,
      redirect: redirectFor(user.status),
    });
  } catch (err) {
...
app.get('/api/media/:requestId/:filename', (req, res) => {
  ...
  const safeRequestId = path.basename(String(requestId));
...
// ⚠️ /api/requests/all должен быть зарегистрирован строго выше /api/requests/:id
  ...
  try {
    ...
    if (!rows.length) {
      return res.status(404).json({
        success: false,
        message: `Заявка № ${requestId} не найдена`,
      });
    }
...
// ===================== SPA FALLBACK =====================
// Совместимо и с Express 4, и с Express 5 без wildcard '*'
app.use((req, res, next) => {
